...
class Matching:
    ...
    def _compute_match_statistics(
        ...
        return MatchStatistics(
            ...
            mentee_match_percent=100.0 * len(matched_mentee_emails) / len(unique_mentee_emails),
            mentor_match_percent=100.0 * len(matched_mentor_emails) / len(unique_mentor_emails),
            median_possible_mentors_for_mentee=float(median(len(value) for value in mentees_to_preferences.values())),
            median_possible_mentees_for_mentor=float(median(len(value) for value in mentors_to_preferences.values())),
            median_matched_mentor_rank_for_mentee=float(median(mentee_ranks)),
            median_matched_mentee_rank_for_mentor=float(median(mentor_ranks)),
            same_state_pair_count=same_state_pair_count,
            total_pair_count=total_pair_count,
            median_seniority_difference=float(median(seniority_differences)),
    ...
    def _gale_shapley(
        ...
        proposer_preferences = deepcopy(proposers)
        acceptor_preferences = deepcopy(acceptors)
